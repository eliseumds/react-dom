...
function resolveCurrentlyRenderingFiber() {
  !(currentlyRenderingFiber$1 !== null) ? invariant(false, 'Hooks can only be called inside the body of a function component.') : void 0;
  {
    ...
    !!isInHookUserCodeInDev ? warning$1(false, 'Hooks can only be called inside the body of a function component. ' + 'Do not call Hooks inside other Hooks. For more information, see ' + 'https://fb.me/rules-of-hooks') : void 0;
  }
  ...
  // the first mismatch is coming from
  {
    if (currentHookMismatchInDev !== null) {
      ...
      if (!didWarnAboutMismatchedHooksForComponent.has(componentName)) {
        ...
        var columnLength = Math.max.apply(null, previousOrder.map(function (hook) {
          return hook.length;
        }).concat('   Previous render'.length));

        var hookStackHeader = padEndSpaces('   Previous render', columnLength) + '    Next render\n';
...
function renderWithHooks(current, workInProgress, Component, props, refOrContext, nextRenderExpirationTime) {
  ...
  do {
    ...
    {
      if (current !== null && workInProgressHook !== null && currentHook === null) {
        warning$1(false, '%s: Rendered more hooks than during the previous render. This is ' + 'not currently supported and may lead to unexpected behavior.', getComponentName(Component));
      }
...

  {
    if (currentHookMismatchInDev === null) {
      if (currentHookNameInDev !== hook._debugType) {
        currentHookMismatchInDev = new Error('tracer').stack.split('\n').slice(4).join('\n');
      }
    }
  }
  return nextHook;
...
function useEffectImpl(fiberEffectTag, hookEffectTag, create, deps) {
  ...
  workInProgressHook.memoizedState = pushEffect(hookEffectTag, create, destroy, nextDeps);
  {
    currentHookNameInDev = null;
  }
}
...
function useImperativeHandle(ref, create, deps) {
  ...
  var nextDeps = deps !== null && deps !== undefined ? deps.concat([ref]) : [ref];
